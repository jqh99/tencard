...
class GenerateData extends Command
{
    ...
    public function nameInDatabase($num = 100){
        ...
        foreach ($nameArrayNew as $k=>$v){
            echo $v."----".$this->genUserNumber().PHP_EOL;
            $id = DB::table('user')->insertGetId([
                'username'=>$this->genUserNumber(),
                'name'=>$v,
                'password'=>'$2y$10$zCRlYpETA7zdg1TLdS7OT.c1EA2dfw9ZzCrVZgC3bALW2bmx5FucG'
            ]);
            $j = $k*10;
            for($i=0;$i<10;$i++){
                DB::table('card')->insert([
                   'user_id'=>$id,
                   'content'=>$this->xljt[$j],
                    'like_count'=>mt_rand(10,1000),
                    'view_count'=>mt_rand(100,10000),
                    'create_time'=>time()-mt_rand(1000,100000000)
                ]);
                $j++;
            }
            if($k==9){
    ...
    public function getXljt(){
        $file = fopen(base_path().DIRECTORY_SEPARATOR."xljt.txt", "r");

        $i=0;
//输出文本中所有的行，直到文件结束为止。
        while(! feof($file))
        {
            $this->xljt[$i]= fgets($file);//fgets()函数从文件指针中读取一行
            $i++;
        }
        fclose($file);
    }
